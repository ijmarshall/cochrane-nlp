...
class BiLearner():
    ...
    def reset(self, seed="regex"):
        # resets everything but keeps computationally expensive features which don't need to be regenerated
        # (as long as they haven't changed)
    ...
    def seed_y_annotations(self, annotation_viewer, hide_reader_ids, test_reader_ids):
        ...
        for study in range(len(self.annotation_viewer)):
            ...
            if study in test_reader_ids:
                test_biviewer_ids.append(biview_id)

            if study in hide_reader_ids:
                hide_biviewer_ids.append(biview_id)
    ...
    def test_pubmed(self, model, display_preds=False):
        ...
        # print set(self.visible_pubmed_ids) & set(self.test_pubmed_ids)

        return self.test(model, self.data["X_pubmed"][self.test_pubmed_ids], self.data["words_pubmed"][self.test_pubmed_ids], self.data["study_id_lookup_pubmed"][self.test_pubmed_ids], display_preds=display_preds)


    def test(self, model, X_test, words_test, joint_from_view_index, display_preds):
        preds = model.predict_proba(X_test)[:,1] # predict unknowns
