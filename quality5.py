...
class DataFilter(object):
    ...
    def _get_available_ids(self):
        ...
        return [i for i, doc in enumerate(data_instance.data)]
...
class SentFilter(DomainFilter):
    ...
    def Xy(self, doc_indices):
        ...
        for i in doc_indices:
            doc_i = self.data_instance.data[i]
            for sent_i, sent_span in enumerate(doc_i["sent-spans"]):
                X.append(doc_i["doc-text"][sent_span[0]:sent_span[1]])
                if sent_i in doc_i["sent-y"][self.domain]:
                    y.append(1)
                else:
                    y.append(-1)
...
class BinaryMetricsRecorder(object):
    ...
    METRIC_FUNCTIONS = [sklearn.metrics.f1_score, sklearn.metrics.precision_score,
    ...
    def add_preds_test(self, preds, test, domain="default"):
        ...
        add a fold of data
        """
        fold_metric = {metric_name: metric_fn(preds, test) for metric_name, metric_fn
...
def simple_model_test(data_filter=DocFilter):
    ...
    for domain in dat.CORE_DOMAINS:
        ...
        for train, test in kf:
            ...
            X_test_d, y_test = docs.Xy(uids[train])
