...
from ..ml.modhashvec2 import ModularVectorizer, InteractionHashingVectorizer
...
class RoBData:
    ...
    def generate_data(self, doc_level_only=False, force_cache_refresh=True, dont_save=False, skip_small_files=False):
        ...
        try:        
            ...
            assert(force_cache_refresh==False)
            self.load_data()
        except:
    ...
    def save_data(self):
        filename=os.path.join(cochranenlp.PATH, "data", "risk_of_bias_cache.pck")
        with open(filename, 'wb') as out_f:
            pickle.dump(self.data, out_f)

    def load_data(self):
        filename=os.path.join(cochranenlp.PATH, "data", "risk_of_bias_cache.pck")
        with open(filename, 'rb') as out_f:
            self.data = pickle.load(out_f)
    ...
    def y(self, doc_indices, pmid_instance=0, domain=None):
        ...
        for i in doc_indices:
            ...
            for doc_domain, judgements in doc_i["sent-y"].iteritems():
                ...
                sent_y[doc_i["sent-y"][doc_domain]] = 1 # except these ones
                y = np.append(y, sent_y)
    ...
    def iter_pmid(self, doc_indices, pmid_instance=0, hide_interactions=False, domain=None):
        ...
        for i in doc_indices:
            ...
            for doc_domain, judgements in doc_i["sent-y"].iteritems():
                ...
                # add sents once per each domain we have sent info for
                for sent in doc_i["sent-spans"]:
                    yield i # generate pubmed id


    def X(self, doc_indices, pmid_instance=0, domain=None):
        return self.X_i(doc_indices=doc_indices, pmid_instance=0, hide_interactions=True, domain=domain)
