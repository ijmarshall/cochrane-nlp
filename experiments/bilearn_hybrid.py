...
class BiLearner():
    ...
    def seed_y_annotations(self, annotation_viewer, hybrid=False):
        """
        initialises the joint y vector with data from manually annotated abstracts
        filter_ids = ids of the MergedTaggedAbstractReader to pay attention to
        hybrid: set to True if want to combine manually annotated with regex seed data
        """
        ...
        for study in range(len(self.annotation_viewer)):
            ...
            self.answers[biview_id] = number
            self.data["y_lookup_init"][biview_id] = number # we can label all in annotated mode since the 'hidden' results
            # are hidden by the filter later, not at this stage

        if hybrid:

            logging.info("Generating seed data from regular expression for hybrid model")
            ...
            for study_id, (cochrane_dict, pubmed_dict) in enumerate(self.biviewer):
                ...
                # matches += re.findall('(?:[Ww]e randomi[sz]ed )([1-9][0-9]*) (?:\w+ )*(?:participants|men|women|patients)', pubmed_text)
                # matches += re.findall('(?:[Aa] total of )([1-9][0-9]*) (?:\w+ )*(?:participants|men|women|patients)', pubmed_text)
                if len(matches) == 1 and self.data["y_lookup_init"][study_id] == -1:
                    self.data["y_lookup_init"][study_id] = int(matches[0])
                    counter += 1
...
def cross_validate():
    ...
    # set up parameters
    C_candidates = np.logspace(-0.5, 0.5, 4)
    testing_folds = 4
    tuning_folds = 2

    testing_fold_size = 0.25
    tuning_fold_size = 0.25
    ...
    CV_main = ShuffleSplit(len(annotated_abstracts), n_iter=testing_folds, test_size=testing_fold_size)

    bilearn.seed_y_annotations(annotation_viewer=annotated_abstracts, hybrid=False)

    for B_indices, A_indices in CV_main:

        print "SUPERVISED"

        CV_tune = ShuffleSplit(len(B_indices), n_iter=tuning_folds, test_size=tuning_fold_size)
        supervised_metrics_l = []

        # get the indices in B of B1 and B2
        for B2_B_indices, B1_B_indices in CV_tune:
            ...
            print "b1, b2", len(B1_indices), len(B2_indices)

            supervised_metrics_l_row = []
            ...
            # TODO implement this:
            bilearn.reset(hide_reader_ids=hidden_abstract_ids,
                          test_reader_ids=B1_indices)


            for C_i in C_candidates:

                model = bilearn.learn_pubmed(C=C_i, update=False) # don't update pool of labelled data at this step
                metrics_i = bilearn.test_pubmed(model)



                supervised_metrics_l_row.append(metrics_i[optimisation_type]) # change metrics_var to whatever it's going to be

                
            supervised_metrics_l.append(supervised_metrics_l_row)

        supervised_metrics = np.array(supervised_metrics_l) # convert nested lists to np.array
        C_max_indices = supervised_metrics.argmax(axis=1)
        C_max_values = C_candidates[C_max_indices]


        C_max_supervised = np.mean(C_max_values)

        print "Optimal C for iteration 0: %f" % (C_max_supervised,)
        ...
        model = bilearn.learn_pubmed(C=C_max_supervised, update=False) # don't update pool of labelled data at this step
    ...
    print "OVERALL - SUPERVISED"
    print "f1", np.mean([i["f1"] for i in metrics])
    print "precision", np.mean([i["precision"] for i in metrics])
    ...
    output = {"C_candidates": C_candidates,
              "tuning_fold_size": tuning_fold_size,
              "testing_fold_size": testing_fold_size,
              "n_iter_max": n_iter_max,
              "optimisation_type": optimisation_type,
              "metrics": metrics}

    with open('hybrid_brief_supervisedpart.log', 'wb') as f:
        pprint(output, f)
    ...
    for B_indices, A_indices in CV_main:
        ...
        for B2_B_indices, B1_B_indices in CV_tune:
            ...
            ####
            ## lookup biviewer IDs for A and B2
            ##  and hide these from the pool of unlabelled data
            ####
            hidden_abstract_ids = np.append(A_indices, B1_indices)
        ...
        C_max_index, n_max = C_n_list[middle_index]
    ...
    with open('hybrid_brief.log', 'wb') as f:
